...
export type PlaybookColumn = {
  netSuccesses: number
  results: readonly [PlaybookResult] | readonly [PlaybookResult, PlaybookResult]
}
...
export function rowEffectsForPick(
  ...
  if (id === 'kd' && kdAlreadyTakenBeforePick(wrapPicks, attackIndex, pickIndex)) {
    return { tacBonusForLater: 0, defReductionForLater: 0 }
  }
  ...
  const u = gbFollowUpUsageBeforePick(wrapPicks, gbFollowUps, attackIndex, pickIndex)
  if (u.so && u.stagger) {
    return { tacBonusForLater: 0, defReductionForLater: 0 }
  }
